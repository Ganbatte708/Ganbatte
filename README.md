# Ganbatte - About me.
## Personal profile
### My career history
#### Details
An accomplished senior IT executive with 35+ years experience:
  Leader in digital transformation and innovation.
  MNCs - legal tech, pharmaceutical, consumer health, crop science, chemical, banking, and retail. 
  Experience: Leading large-scale projects infrastructure and applications, 
  Business partnering and stakeholder management
  Budget oversight
  Coach
Technologies:
  Coding: COBOL, NATURAL, CLP, Python, C++
  Network: CISCO
  Firewall: Pix, Checkpoint
  ITSM: Service Now
  Systems: AS/400, IBM S/370, Dell
Education: 
  Post-graduate diploma in Systems Analysis with NUS-ISS.
  B. Sc. Physics
Hobbies: Amateur violinist, reading, 
Christian Religion: Presbyterian
